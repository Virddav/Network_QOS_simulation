...
class Djikstra:
    ...
    def set_noeud(self):
        ...
        self.choisir_noeud_principal()
        self.generer_table_routage()
    
    def choisir_noeud_principal(self):
        distances_moyennes = {}
        for noeud in self.graph.nodes():
            distances = nx.single_source_dijkstra_path_length(self.graph, noeud)
            distances_moyennes[noeud] = sum(distances.values()) / (len(self.graph.nodes()) - 1)
        self.noeud_principal = min(distances_moyennes, key=distances_moyennes.get)
        print(f"Nœud principal choisi : {self.noeud_principal} avec distance moyenne minimale = {distances_moyennes[self.noeud_principal]}")

    def generer_table_routage(self):
        for destination in self.graph.nodes():
            if self.noeud_principal != destination:
                chemin = self.calculer_chemin_optimal(self.noeud_principal, destination)
                if chemin:
                    self.table_routage[destination] = chemin

    def rechercher_dans_table(self, destination):
        return self.table_routage.get(destination)

    def show_state(self):
        for elem in self.list_noeud:
            elem.show_voisins()

    def calculer_chemin_optimal(self, source, destination):
        try:
    ...
    def show_table_routage(self):
        print("Table de routage (depuis le nœud principal) :")
        for destination, chemin in self.table_routage.items():
            print(f"Chemin vers {destination} : {chemin}")
